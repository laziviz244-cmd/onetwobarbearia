...
export default function AdminFinanceiro() {
  ...
  return (
    <AdminLayout>
      <h1 className="font-montserrat font-bold text-2xl tracking-tight mb-4" style={{ color: "#F9FAFB" }}>Financeiro</h1>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="rounded-2xl p-4 flex flex-col gap-1" style={cardStyle}>
          <TrendingUp className="h-5 w-5 mb-1" style={{ color: "#22C55E" }} />
          <span className="font-montserrat font-bold text-lg tabular-nums" style={{ color: "#F9FAFB" }}>R$ {totalPayments.toFixed(0)}</span>
          <span className="text-[10px] font-opensans" style={{ color: "#9CA3AF" }}>Receitas</span>
        </div>
        <div className="rounded-2xl p-4 flex flex-col gap-1" style={cardStyle}>
          <TrendingDown className="h-5 w-5 mb-1" style={{ color: "#EF4444" }} />
          <span className="font-montserrat font-bold text-lg tabular-nums" style={{ color: "#F9FAFB" }}>R$ {totalExpenses.toFixed(0)}</span>
          <span className="text-[10px] font-opensans" style={{ color: "#9CA3AF" }}>Despesas</span>
        </div>
        <div className="rounded-2xl p-4 flex flex-col gap-1" style={cardStyle}>
          <DollarSign className="h-5 w-5 mb-1" style={{ color: "#2563EB" }} />
          <span className="font-montserrat font-bold text-lg tabular-nums" style={{ color: profit >= 0 ? "#F9FAFB" : "#EF4444" }}>R$ {profit.toFixed(0)}</span>
          <span className="text-[10px] font-opensans" style={{ color: "#9CA3AF" }}>Lucro</span>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={() => tab === "pagamentos" ? setPaymentDialogOpen(true) : setExpenseDialogOpen(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-montserrat font-semibold text-white transition-all hover:brightness-110" style={{ background: "#2563EB" }}>
